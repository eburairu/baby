...
**学び:**
- スケジュール機能（`app/routers/schedule.py`）において、APIリクエスト・レスポンススキーマ（`ScheduleCreate`, `ScheduleResponse`）が仕様書（`schedule_tracker.md`）から完全に欠落していました。
- 他の記録系機能（睡眠やおむつなど）の仕様書では、TypeScriptインターフェース形式でスキーマが定義されていますが、スケジュール機能ではデータモデル（DBスキーマ）の定義にとどまり、APIレベルでのインターフェース定義（どの項目が省略可能か、等）が存在しない状態でした。

**アクション:**
- `schedule_tracker.md` に「リクエスト/レスポンススキーマ」のセクションを追加し、`extends`キーワードを活用して`ScheduleCreate`と`ScheduleResponse`を定義しました。
- 今後は、機能のデータモデルが存在していても、API層でのリクエスト/レスポンス形式が別途TypeScriptインターフェースとして正しくドキュメント化されているかを必ず確認するようにします。
