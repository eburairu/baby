...
## 2026-03-03 - 陣痛タイマー仕様書のPATCH対応漏れ
**学び:** 陣痛タイマー（`contraction.py`）などの新しいエンドポイントの追加時に、API定義 (`PATCH /api/contractions/{id}`) の記載は行われても、対応するPydanticの更新スキーマ（`ContractionUpdate`）が仕様書のリクエスト/レスポンススキーマの項から漏れやすい。機能の変更時に付随する型の更新はコード側では完結しているが、ドキュメント側では後回しにされる傾向がある。
**アクション:** 新しい機能やエンドポイントの追加仕様書を見直す際は、エンドポイント自体の説明だけでなく、Pydanticの定義 (`Create`, `Update`, `Response`) 全てが揃って仕様書に転記されているかまで確認する。
