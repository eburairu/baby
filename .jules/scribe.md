...
## 2025-03-01 - [仕様と実装の乖離パターン: PydanticとTypeScript Schemaの継承不足]
**学び:** バックエンド（Pydantic）で Base モデルや Create モデルを継承して Response モデルを定義しているにも関わらず、仕様書側の TypeScript インターフェースでは全フィールドを再定義してしまっている（または一部の Create/Update モデルの定義が抜け落ちている）パターンが散見される。
**アクション:** 今後の仕様書更新では、TypeScript インターフェースを `extends` を使ってDRYに記述し、バックエンドのスキーマ設計と一致させるようにする。また、新しいAPI仕様を追加する際は、Create/Update/Response のセットが全て記載されているかを確認する。
