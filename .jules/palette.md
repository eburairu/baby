...
## 2026-02-25 - [Button Loading State for Icons] **学び:** アイコンのみの小さなボタンにおいて、標準の「スピナー＋コンテンツ」のローディング表示はレイアウト崩れや視覚的なノイズを引き起こす。 **アクション:** `WidgetQuickButton` に `hideContentOnLoading` オプションを追加し、ローディング中はコンテンツを隠してスピナーのみを表示することで、視覚的な安定性を保ちつつ `aria-busy` でアクセシビリティを確保するパターンを確立した。

## 2026-02-26 - [Nesting Interactive Elements] **学び:** Next.jsの `<Link>` をスタイリングのために `<Button>` でラップすると、`<a>` タグの中に `<button>` タグが入る不正なHTML構造になり、アクセシビリティの問題を引き起こす。 **アクション:** shadcn/ui (Radix UI) の `Button` コンポーネントの `asChild` プロパティを活用し、 `<Button asChild><Link>...</Link></Button>` のように実装することで、セマンティックな `<a>` タグを維持しつつボタンのスタイルを適用するパターンを徹底する。
