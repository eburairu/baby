...
## 2024-03-12 - [赤ちゃん情報ポップアップのタブ切り替えにARIA属性を追加]
**学び:** タブを切り替えるUIにおいて、ボタンの現在の選択状態がスクリーンリーダーに伝わらない状態（`aria-pressed` や `aria-selected` がない状態）でした。単に見た目を変えるだけでなく、`aria-pressed` を使ってアクティブ状態を明示することで、よりアクセシブルなタブナビゲーションになります。
**アクション:** 今後、自作のタブコンポーネントやトグル要素を実装する際には、必ず `aria-pressed` や適切な `aria-label` を追加して状態変化を通知するようにします。
