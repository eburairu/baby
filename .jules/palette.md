...
## 2024-03-10 - リストアイテムのボタンに対する視覚的ツールチップ
**学び:** `ActivityItem` などのリストアイテム全体がクリッカブルな要素 (`<button>`) となっている場合、スクリーンリーダー向けの `aria-label` は設定されていても、マウスホバー時に詳細なアクション内容を示すネイティブツールチップ (`title` 属性) が欠けていることがある。
**アクション:** 領域の広いクリッカブル要素であっても、特に詳細表示や特定のアクションを伴う場合は、マウスユーザー向けの補助として `title` 属性を付与し、インタラクションの意図をより明確に伝達する。
