...
def run(playwright):
    ...
    # Use filter to get the widget specifically
    feeding_widget = page.locator("div").filter(has_text="授乳").first
    milk_btn = feeding_widget.get_by_label("ミルクを記録")
    breast_btn = feeding_widget.get_by_label("母乳を記録")
    ...
    diaper_widget = page.locator("div").filter(has_text="おむつ").first
    diaper_wet_btn = diaper_widget.get_by_label("おしっこ")
    diaper_dirty_btn = diaper_widget.get_by_label("うんち")
