...
- **セキュリティ対策**:
    - **レートリミット**: 1ユーザーあたり 60秒間に10回 まで（DoS攻撃およびストレージ枯渇防止）。
    - **マジックバイト検証**: 拡張子や `Content-Type` ヘッダーを信頼せず、ファイルシグネチャ（マジックバイト）を検証してファイル形式を特定する。
- **認証**: `get_current_user()` による認証必須（`Depends`）
- **Request**: `multipart/form-data`（`UploadFile`）
- **処理ロジック**:
  1. **予備チェック**: `content_type` が `image/` で始まることを確認（ただし信頼しない）。
  2. **サイズチェック**: ファイル内容を全て読み込み、5MB 超なら `400 Bad Request`。
  3. **形式特定 (Magic Bytes)**: ファイルの先頭バイトを読み取り、以下の形式であることを検証する。
     - JPEG, PNG, GIF, WebP
     - 上記以外は `400 Bad Request`（HTMLやPHPなどの偽装アップロードを防止）。
  4. **キー生成**: 検出された形式に対応する正しい拡張子を使用し、`{uuid4}.{detected_ext}` でオブジェクトキーを生成する（元のファイル名は使用しない）。
  5. boto3 クライアントを初期化
