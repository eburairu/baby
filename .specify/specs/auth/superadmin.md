...
Botoro のシステム全体を管理するための権限（SuperAdmin）を定義する。
...
| 機能カテゴリ | 具体的な操作 | 内容 |
| :--- | :--- | :--- |
| **ダッシュボード** | システム統計の閲覧 | 全ユーザー数、全家族数、全記録数、直近24時間のアクティブユーザー数の確認 |
| **ダッシュボード** | 最近の監査ログの閲覧 | システム内で発生した重要な操作（家族作成、権限変更等）の履歴確認 |
| **家族管理** | 家族一覧の閲覧 | システム内の全家族の一覧表示、検索 |
| **家族管理** | 家族詳細の閲覧 | 家族名、作成日、所属メンバー、登録されている赤ちゃんの確認 |
| **家族管理** | 家族の新規作成 | 新しい家族をシステムに追加し、招待コードを発行する |
...
| **監査ログ** | 全監査ログの閲覧 | システム全体の操作ログのページネーション表示、実行ユーザーやIPアドレスの確認 |
...
- `/admin`: 管理者ダッシュボード（統計カード、最近の監査ログ）
- `/admin/families`: 家族一覧・詳細（新規作成ダイアログ、検索機能、詳細表示シート）
- `/admin/users`: ユーザー一覧・権限管理（SuperAdmin トグルの操作）

### 管理画面の UI/UX ガイドライン（モバイル対応）

- 他のアプリ内画面と一貫した `shadcn/ui` ベースのモダンなデザインを採用する。
- デスクトップでは左サイドバー、モバイルではヘッダーメニュー（ハンバーガーメニュー等）でナビゲーションを提供する。
- メインコンテンツ領域はモバイル環境で左右の余白（padding）を最適化し、`p-4 md:p-6 lg:p-8` 等を標準とする。
- グリッドレイアウトはモバイルで `grid-cols-1`、タブレットで `grid-cols-2` などを適宜設定し、横スクロールが発生しないように配慮する。
