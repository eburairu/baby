...
## 2.1 セッションハイジャック対策 (IP/User-Agent バインディング)
- `UserSession` 作成時にリクエスト元の `IPアドレス` と `User-Agent` (またはそのハッシュ値) を記録する。
- 認証時にリクエストヘッダーから取得したIPアドレス・User-Agentと、DBに保存された情報が大きく乖離している（全く異なる環境からのアクセス等）場合、セッションを無効化して再ログインを要求する。これによりCookie流出時の不正利用を防ぐ。
...
| 登録リンク | Register | 新規登録 |
| パスワード忘れ案内 | — | パスワードをお忘れの場合は、家族の管理者にお問い合わせください。 |
| エラー (必須) | Username is required | ユーザー名を入力してください |
| エラー (必須) | Password is required | パスワードを入力してください |
| エラー (失敗) | Login failed | ログインに失敗しました |

#### 5.1.1 パスワードを忘れた場合の案内

- **表示箇所**: ログイン画面 CardFooter（「アカウントをお持ちでないですか？」テキストの下）
- **表示形式**: `text-sm text-muted-foreground`（小さめのグレー系テキスト）
- **表示内容**: `パスワードをお忘れの場合は、家族の管理者にお問い合わせください。`
- **セルフサービスリセットは実装しない**: メール送信等によるパスワードリセットは提供しない。
- **対処方法**: ファミリーの ADMIN が「設定 > メンバー管理」から対象ユーザーのパスワードを再発行できる（仮パスワードを生成してメンバーに伝達）。
