...
- **過去の日付を遡って日誌を残したい**
    - ユーザーは、まだ日誌を生成していない過去の日（昨日・先週など）を選択して、その日の記録を元に日誌を後から生成したい。
    - **Acceptance Criteria**: 日誌一覧画面から過去の日付を指定し、その日の記録に基づく日誌を生成・保存できること。未来の日付は選択不可。
...
└─────────────────────────────────────────┘
  ...
  ┌───────────────────────────────────────┐
  │  📅 日付を指定して生成                 │
  │  [2026-02-14    ▼]  [日誌を生成]      │  ← 日付ピッカー + ボタン
  │  ※ 未来の日付は選択不可               │
  └───────────────────────────────────────┘
  ↑ 生成済みの日付は「再生成」、未生成は「日誌を生成」とラベル変化
...
- **日付ピッカー**: `<input type="date">` または shadcn/ui の `Calendar` + `Popover` を使用
- **デフォルト値**: 今日の日付（JST）
- **選択可能範囲**: 過去〜今日（未来日付は `max` 属性で制限）
- **ボタンラベル**:
  - 選択した日付に既存の日誌が存在しない → 「日誌を生成」
  - 選択した日付に日誌が存在する → 「再生成」（`is_edited=true` の場合はグレーアウトまたは警告付き）
- **`is_edited=true` の日誌への再生成**: ダイアログで「手動編集済みの日誌があります。再生成すると上書きされます。続けますか？」と確認を求める
...
- [x] `frontend/app/(dashboard)/diary/page.tsx` 作成
    ...
    - [x] 削除確認ダイアログ
- [ ] `frontend/app/(dashboard)/diary/page.tsx` 更新（過去日付生成対応）
    - [ ] 日付ピッカー UI 追加（`max` 属性に今日の日付を設定して未来日付を選択不可に）
    - [ ] ボタンラベルの動的切替（「日誌を生成」/ 「再生成」）
    - [ ] `is_edited=true` の日誌を再生成しようとした際の確認ダイアログ
    - [ ] 生成後に日誌一覧を `mutate()` で更新
