...
### 1.3 共通の状態表示
- **初期ロード**: 赤ちゃん情報や権限の読み込み中は `PageLoading` を表示。
- **データロード**: 機能固有のデータの読み込み中は `BabyBottleLoading` スピナーを表示。
...
ページ内は上から以下の順序で配置することを推奨する。

1.  **統計/サマリー (Stats/Summary)**: 当日の合計値や直近の状況を表示。
2.  **アドバイス (TipsCard)**: その記録項目に関する育児アドバイスを表示。
3.  **入力フォーム (Action/Form)**: `canWrite` が `true` の場合に表示。
