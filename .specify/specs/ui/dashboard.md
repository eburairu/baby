...
- **BabyProfileCard**: 出生前は月齢の代わりに予定日情報を表示。また、赤ちゃんの特徴（characteristics）が登録されている場合は、プロフィール内に引用スタイルで表示する。
