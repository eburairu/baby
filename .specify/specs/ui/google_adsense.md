...
| 場所 | 条件 | 説明 |
|------|------|------|
| `RecentActivityFeed` コンポーネント内 | 記録5件ごとに1回 | 5件目・10件目・15件目… の直後に広告を挿入 |

- **挿入ロジック**: `recent.map((record, index) => ...)` の内部で `(index + 1) % 5 === 0` の条件が `true` の時に広告ユニットを表示する。
...
2. `recent.map()` の各アイテムを `React.Fragment` でラップし、5件ごとに `AdUnit` を挿入する。
...
| 確認項目 | 期待結果 |
|----------|----------|
| 記録が5件以上ある場合 | 5件目の直後に広告エリアが表示される |
| 記録が5件未満の場合 | 広告は表示されない |
| 無限スクロールで10件目が表示された場合 | 10件目の直後にも広告が表示される |
