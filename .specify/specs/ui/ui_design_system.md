...
3. **一貫性の維持**: 新しいコンポーネントを追加する際も「この要素は六角形を取り入れることでUX/UIの統一感が高まるか」を常に検討すること。
...
ダッシュボードはサブページと異なり、ヘッダーは `BabyProfileCard` が担う。
グローバルヘッダーには以下の要素を配置する。
...
> **注**: 以前ヘッダーに配置されていた「Logout」ボタンは、利用頻度が低いため設定画面内へ移動した。

#### 赤ちゃんバッジの操作設計

| 条件 | 表示・動作 |
|------|-----------|
| 赤ちゃんが1人 | shadcn/ui `Badge` を静的表示（クリック不可） |
| 赤ちゃんが2人以上 | `<button>` にラップし `ChevronDown` アイコンを追加。クリックで shadcn/ui `DropdownMenu` を表示 |

**DropdownMenu の設計**:

- `align="end"` で右揃えに配置
- 各アイテムに赤ちゃん名を表示し、現在選択中の赤ちゃんには `Check` アイコン（`opacity-100`）、非選択は `opacity-0` で整列を維持
- 選択時に `useBabyStore` の `setSelectedBabyId` を呼び出してグローバル状態を更新
