...
- **表示位置**: 統計サマリーカード（F3）の直下に配置。
- **グラフ種別**: `recharts` の `ComposedChart`（棒グラフ + 折れ線グラフの混合）。
- **表示期間**: 直近7日間（今日を含む）。
- **データ系列**:
    - **棒グラフ（回数）**: 左軸 (`yAxisId="count"`)。色: rose（ダーク: `#fb7185` / ライト: `#f43f5e`）。
    - **折れ線（母乳時間・分）**: 右軸 (`yAxisId="amount"`)。`breastMin > 0` のデータが存在する日がある場合のみ表示。色: indigo（ダーク: `#818cf8` / ライト: `#6366f1`）。
    - **折れ線（ミルク量・ml）**: 右軸 (`yAxisId="amount"`)。`bottleMl > 0` のデータが存在する日がある場合のみ表示。色: amber（ダーク: `#fbbf24` / ライト: `#f59e0b`）。
- **空状態**: 7日間すべて記録がない場合は「記録がありません」を表示。
- **ダークモード対応**: `useTheme()` で `resolvedTheme` を取得し、グリッド・テキスト・ツールチップ・バーの色をテーマに合わせて切り替える。
- **実装ファイル**:
    - `frontend/components/feeding/FeedingChart.tsx` — グラフコンポーネント
    - `frontend/lib/feedingUtils.ts` — `calculateDailyStats()` 関数（7日分の日次集計）
