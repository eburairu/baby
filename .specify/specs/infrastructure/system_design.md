...
- **Validation**: 未来日時のバリデーションを行う場合、クライアントとサーバーの時刻同期のズレを考慮し、許容誤差（デフォルト5分、定数 `FUTURE_DATE_BUFFER_MIN` で定義）を設けます。詳細な定数管理については [マジックナンバー排除規約](../development/magic_number_policy.md) を参照してください。
