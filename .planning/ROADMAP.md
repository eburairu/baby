...
- [x] 05-02-PLAN.md — Alembic マイグレーション and API ルーターの実装
...
**Milestone Goal:** LPおよびコラムページのダークモード対応を完了し、全デバイスで快適な閲覧体験を提供する

## Phase Details

### Phase 18: コラムダークモード対応
**Goal:** 育児コラム関連ページおよびLPのティーザーセクションをダークモードに完全対応させる
**Requirements:** UI-DARK-01
**Success Criteria** (what must be TRUE):
  1. ダークモード時に LP の育児コラムセクションが `dark:bg-zinc-900/50` の背景で表示される
  2. `/column` ページが `dark:bg-zinc-950`（または適切なダーク色）の背景で表示される
  3. コラム記事ページ（`/column/[slug]`）の各セクションがダークモード用の配色で表示される
  4. `CATEGORY_COLORS` がダークモードでも視認性の高い配色に切り替わる
  5. `scripts/verify_all.sh` がパスする

**Plans:** 3 plans
- [ ] 18-01-PLAN.md — LandingColumnTeaser.tsx のダークモード対応とカテゴリカラー共通化
- [ ] 18-02-PLAN.md — app/column/page.tsx のダークモード対応
- [ ] 18-03-PLAN.md — app/column/[slug]/page.tsx のダークモード対応と検証
...
| 18. コラムダークモード対応 | v1.4 | 0/3 | In Progress | - |
