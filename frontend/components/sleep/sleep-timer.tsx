    ...
    return (
        <Card className="bg-white rounded-2xl shadow-sm border-0 overflow-hidden">
            <CardContent className="p-6 flex flex-col items-center justify-center space-y-6">
                <div className={`relative flex items-center justify-center w-32 h-32 rounded-full transition-colors duration-500 ${isSleeping ? "bg-indigo-100" : "bg-orange-50"}`}>
                    {isSleeping ? (
                        <Moon className="w-16 h-16 text-indigo-500 animate-pulse" />
                    ) : (
                ...
                <div className="text-center space-y-1">
                    ...
                    {isSleeping && (
                        <p className="text-4xl font-bold text-gray-800 tracking-tight font-mono">
                            {elapsed}
                        </p>
                    )}
                </div>
